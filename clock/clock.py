...
def minute_change(device):
    ...
    def helper(current_y):
        with canvas(device) as draw:
            text(draw, (0, 0), hours, fill="white", font=proportional(BENGALI_FONT))
            text(draw, (15, 0), ":", fill="white", font=proportional(TINY_FONT))
            text(draw, (18, current_y), minutes, fill="white", font=proportional(BENGALI_FONT))
        time.sleep(0.1)
...
def animation(device, from_y, to_y):
    ...
    while current_y != to_y:
        with canvas(device) as draw:
            text(draw, (0, current_y), hourstime, fill="white", font=proportional(BENGALI_FONT))
    ...
    while True:
        ...
        if sec == 59:
            ...
            with canvas(device) as draw:
                ...
                text(draw, (18, 0), minutes, fill="white", font=proportional(BENGALI_FONT))
            time.sleep(0.5)
